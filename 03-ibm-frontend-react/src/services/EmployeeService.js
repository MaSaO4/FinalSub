...
const BASE_URL = 'http://localhost:9091/Class2';
const EmployeeService = {
    ...
    update: async (employee, eid) => {
        try {
            const response = await axios.patch(`${BASE_URL}/update-emp`, employee);
            return response.data;
        } catch (error) {
            console.error(error);
    ...
    search: async (fName) => {
        try {
            const response = await axios.get(`${BASE_URL}/emp-fname/${fName}`)
            console.log(response.data);
            return response.data;
        } catch (error) {
            console.log(error);
